(() => {
  "use strict";
  const ds=window.DataService,$=id=>document.getElementById(id);let profile=null,currentUser=null,users=[];
  const esc=v=>String(v??"").replace(/[&<>'"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"}[c]));
  const roleLabel=r=>({admin:"Διαχειριστής",editor:"Συντάκτης",viewer:"Θεατής"}[r]||r);
  ...
  async function updateUser(id,changes){const u=users.find(x=>x.id===id);if(!u)return;if(!confirm(`Να ενημερωθεί ο χρήστης ${u.email};`)){render();return;}try{await invoke("update",{user_id:id,...changes});await loadUsers();}catch(e){alert(e.message);render();}}
  async function deleteUser(id){const u=users.find(x=>x.id===id);if(!u)return;const typed=prompt(`ΟΡΙΣΤΙΚΗ ΔΙΑΓΡΑΦΗ του ${u.email}.\nΓράψε ΔΙΑΓΡΑΦΗ για επιβεβαίωση:`);if(typed!=="ΔΙΑΓΡΑΦΗ")return;try{await invoke("delete",{user_id:id});await loadUsers();}catch(e){alert(e.message);}}
  async function resetPassword(id){const u=users.find(x=>x.id===id);if(!u?.email)return;if(!confirm(`Να σταλεί email επαναφοράς κωδικού στο ${u.email};`))return;const{error}=await ds.client.auth.resetPasswordForEmail(u.email,{redirectTo:authRedirect()});if(error)alert(error.message);else alert("Το email επαναφοράς στάλθηκε.");}
  $("openInviteUser")?.addEventListener("click",()=>{$("inviteUserModal").classList.remove("hidden");document.body.classList.add("modal-open");});$("closeInviteUser")?.addEventListener("click",()=>{$("inviteUserModal").classList.add("hidden");document.body.classList.remove("modal-open");});$("inviteUserForm")?.addEventListener("submit",async e=>{e.preventDefault();const m=$("inviteUserMessage");m.textContent="Αποστολή…";try{await invoke("invite",{email:$("inviteEmail").value.trim(),full_name:$("inviteFullName").value.trim(),role:$("inviteRole").value,redirect_to:authRedirect()});m.textContent="Η πρόσκληση στάλθηκε.";e.target.reset();await loadUsers();}catch(error){m.textContent=error.message;m.classList.add("error");}});
})();
