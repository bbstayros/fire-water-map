      ...
      ? rooms.map(room => {
          ...
          return `<article class="operation-room-card ${room.is_active ? "" : "inactive"}">
            <div class="operation-room-card-head">
              ...
              <div class="operation-count ${(activeCrewCount+activeSupportCount)?"has-live":"no-live"}"><strong>${activeCrewCount+activeSupportCount}</strong><span>${activeCrewCount} οχήματα · ${activeSupportCount} υποστήριξη</span></div>
            </div>
            ${vehicles}
            ${roomSupport.length ? `<div class="admin-support-live"><h4>◆ Υποστήριξη (${roomSupport.length})</h4>${roomSupport.map(s=>{const live=s.last_seen_at&&Date.now()-new Date(s.last_seen_at).getTime()<120000;const speed=s.speed_mps!=null?`${Math.round(Number(s.speed_mps)*3.6)} km/h`:"—";const mapLink=s.latitude!=null&&s.longitude!=null?`https://www.google.com/maps?q=${s.latitude},${s.longitude}`:"";return `<article class="admin-vehicle-card ${live?"live":"offline"}"><div class="admin-vehicle-icon">◆</div><div class="admin-vehicle-copy"><div class="admin-vehicle-title"><strong>${escapeHtml(s.full_name)}</strong><span>${live?"Live":"Χωρίς πρόσφατο GPS"}</span></div><div class="admin-vehicle-meta"><span>${escapeHtml(s.support_type)}</span><span>${escapeHtml(s.vehicle_info||"")}</span><span>${speed}</span></div></div>${mapLink?`<a class="vehicle-map-button" target="_blank" rel="noopener" href="${mapLink}">Χάρτης</a>`:""}</article>`;}).join("")}</div>` : ""}
