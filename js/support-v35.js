(() => {
  ...
  function clearLocal(){removeSupportMessaging();requestId=requestToken=accessToken="";["fwm-support-request-id","fwm-support-request-token","fwm-support-access-token","fwm-support-name","fwm-support-type","fwm-support-vehicle"].forEach(k=>localStorage.removeItem(k));approved=false;stopGps();$("supportActiveBar")?.classList.add("hidden");panel("form");}
  ...
  function supportSeenKey(){return `fwm-support-last-seen-${requestId||"current"}`;}
  function setSupportUnread(n){localStorage.setItem("fwm-support-unread-count",String(Math.max(0,n||0)));window.dispatchEvent(new CustomEvent("fwm-support-unread-changed"));}
  ...
  function removeSupportMessaging(){clearInterval(supportMsgTimer);supportMsgTimer=null;$("supportMessageFabV381")?.remove();$("supportMessagingV381")?.remove();localStorage.removeItem("fwm-support-unread-count");window.dispatchEvent(new CustomEvent("fwm-support-unread-changed"));}
  async function loadSupportMessages(){
    const drawer=$("supportMessagingV381");if(!drawer||!accessToken)return;
    try{const{data,error}=await ds.client.rpc("support_messages_v381",{p_access_token:accessToken,p_limit:200});if(error)throw error;const rows=Array.isArray(data)?data:[];const list=drawer.querySelector("[data-list]");list.innerHTML=rows.length?rows.map(m=>`<div class="support-msg ${m.sender_type==="support"?"mine":"theirs"}"><strong>${escapeHtml(m.sender_label||"")}</strong><span>${escapeHtml(m.body||"")}</span>${m.latitude!=null&&m.longitude!=null?`<a target="_blank" rel="noopener" href="https://www.google.com/maps/dir/?api=1&destination=${m.latitude},${m.longitude}">📍 ${escapeHtml(m.point_name||"Πλοήγηση")}</a>`:""}<small>${new Date(m.created_at).toLocaleString("el-GR")}</small></div>`).join(""):"<p>Δεν υπάρχουν μηνύματα ακόμα.</p>";list.scrollTop=list.scrollHeight;
      const incoming=rows.filter(m=>m.sender_type!=="support"&&m.created_at);const newest=incoming.reduce((v,m)=>Math.max(v,new Date(m.created_at).getTime()||0),0);const seen=Number(localStorage.getItem(supportSeenKey())||0);
      if(!drawer.classList.contains("hidden")){if(newest)localStorage.setItem(supportSeenKey(),String(newest));setSupportUnread(0);}else setSupportUnread(incoming.filter(m=>(new Date(m.created_at).getTime()||0)>seen).length);
    }catch(e){console.warn("support messages",e);}
  }
