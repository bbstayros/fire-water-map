(() => {
  ...
  function ensureSupportMessaging(){
    ...
    if(!fab){
      ...
      fab.onclick=()=>{drawer.classList.remove("hidden");loadSupportMessages(true);};drawer.querySelector("[data-close]").onclick=()=>drawer.classList.add("hidden");drawer.querySelector("[data-form]").onsubmit=sendSupportMessage;
    }
  ...
  async function loadSupportMessages(markRead=false){
    ...
    try{const{data,error}=await ds.client.rpc("support_messages_v381",{p_access_token:accessToken,p_limit:200});if(error)throw error;const rows=Array.isArray(data)?data:[];const list=drawer.querySelector("[data-list]");list.innerHTML=rows.length?rows.map(m=>`<div class="support-msg ${m.sender_type==="support"?"mine":"theirs"}"><strong>${escapeHtml(m.sender_label||"")}</strong><span>${escapeHtml(m.body||"")}</span>${m.latitude!=null&&m.longitude!=null?`<a target="_blank" rel="noopener" href="https://www.google.com/maps/dir/?api=1&destination=${m.latitude},${m.longitude}">📍 ${escapeHtml(m.point_name||"Πλοήγηση")}</a>`:""}<small>${new Date(m.created_at).toLocaleString("el-GR")}</small></div>`).join(""):"<p>Δεν υπάρχουν μηνύματα ακόμα.</p>";list.scrollTop=list.scrollHeight;
      const centerRows=rows.filter(m=>m.sender_type!=="support");
      const key="fwm-support-last-read-v392";
      let seen=localStorage.getItem(key);
      const latest=centerRows.reduce((v,m)=>String(m.created_at||"")>v?String(m.created_at||""):v,"");
      if(!seen){ if(latest)localStorage.setItem(key,latest); seen=latest; }
      if(markRead || !drawer.classList.contains("hidden")){ if(latest)localStorage.setItem(key,latest); seen=latest; }
      const unread=centerRows.filter(m=>String(m.created_at||"")>String(seen||"")).length;
      window.dispatchEvent(new CustomEvent("fwm:support-unread",{detail:{count:unread}}));
