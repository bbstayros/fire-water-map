(() => {
  ...
  notificationToggle?.addEventListener("click", async () => {
    ...
    if (source) {
      source.click();
      setTimeout(syncNotificationState, 100);
      setTimeout(syncNotificationState, 600);
      return;
    }
    // Fallback for temporary Support sessions: enable browser permission/push directly.
    try {
      if ("Notification" in window && Notification.permission === "default") {
        await Notification.requestPermission();
      }
      if (window.FwmPush) await window.FwmPush.enable();
      notificationToggle.classList.toggle("is-on", Notification.permission === "granted");
      const label = notificationToggle.querySelector(".state-label");
      if (label) label.textContent = Notification.permission === "granted" ? "ON" : "OFF";
    } catch (e) {
      console.warn("notification enable", e);
    }
  });
  ...
  function syncMessageBadge() {
    ...
    if (!badge) return;
    const isSupport = !!localStorage.getItem("fwm-support-access-token");
    const n = isSupport ? supportUnread : Number(source?.textContent || 0);
    if (badge.textContent !== String(n)) badge.textContent = String(n);
    badge.classList.toggle("hidden", n <= 0);
  }

  window.addEventListener("fwm:support-unread", event => {
    supportUnread = Math.max(0, Number(event.detail?.count || 0));
    syncMessageBadge();
  });
  ...
  mobileMessagesButton?.addEventListener("click", () => {
    ...
    setTimeout(syncMessageBadge, 100);
  });
